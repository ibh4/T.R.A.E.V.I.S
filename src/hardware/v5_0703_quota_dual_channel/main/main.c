                          ...
                          float sweep_deg, int dot_r, uint16_t color)
{
    ...
    for (float deg = start; deg <= end; deg += 0.9f) {
...
{
    for (float deg = start_deg; deg <= end_deg; deg += 0.9f) {
        float rad = deg * M_PI / 180.0f;
        int x = CENTER_X + (int)(radius * cosf(rad));
        int y = CENTER_Y + (int)(radius * sinf(rad));
        fill_circle(buf, x, y, dot_r, color);
                                   ...
                                   int fill_enabled)
{
    ...
    if (fill_enabled) {
        for (float deg = start; deg <= end; deg += 0.28f) {
            float rad = deg * M_PI / 180.0f;
            draw_menu_thick_line(buf,
                                 CENTER_X + (int)((inner_r + 2) * cosf(rad)),
                                 CENTER_Y + (int)((inner_r + 2) * sinf(rad)),
                                 CENTER_X + (int)((outer_r - 3) * cosf(rad)),
                                 CENTER_Y + (int)((outer_r - 3) * sinf(rad)),
                                 1, fill);
        }
    }

    draw_menu_arc_range(buf, outer_r, start, end, 1, stroke);
    draw_menu_arc_range(buf, inner_r, start, end, 1, menu_mix_565(stroke, COL_BLACK, 0.72f));
    ...
    draw_menu_thick_line(buf,
                         ...
                         1, menu_mix_565(stroke, COL_BLACK, 0.70f));
    draw_menu_thick_line(buf,
...
static void draw_menu_avatar(uint16_t *buf, int cx, int cy, int size)
{
    if (!s_states_part || !load_state_frame(0, s_menu_frame)) return;
    const uint16_t *s = (const uint16_t *)s_anim_buf_240;
    int x0 = cx - size / 2;
    int y0 = cy - size / 2;
    int clip_r = size / 2;
    for (int y = 0; y < size; y++) {
        int sy = y * ANIM_H / size;
        if (sy >= ANIM_H) sy = ANIM_H - 1;
        const uint16_t *srow = s + sy * ANIM_W;
        for (int x = 0; x < size; x++) {
            int sx = x * ANIM_W / size;
            if (sx >= ANIM_W) sx = ANIM_W - 1;
            int dx = x - size / 2;
            int dy = y - size / 2;
    ...
    s_menu_frame = (s_menu_frame + 1) % FRAMES_PER_STATE;
}
...
static void draw_menu(void)
{
    ...
    /* 接近 HTML 原型: 默认近黑扇区 + 外圈彩色粗弧, 拖动时只点亮目标方向 */
    draw_menu_arc_range(buf, 212, 208, 332, 4, menu_mix_565(COL_CYAN, COL_BLACK, 0.72f));
    draw_menu_arc_range(buf, 212, -56, 58, 4, menu_mix_565(COL_BRIGHT_GR, COL_BLACK, 0.76f));
    ...
    for (int i = 0; i < 6; i++) {
        ...
        int inner_r = is_active ? 139 : 140;
        int outer_r = is_active ? 195 : 193;
        uint16_t accent = menu_accent_color(i);
        uint16_t edge = is_active ? accent : menu_mix_565(COL_WHITE, accent, 0.10f);
        uint16_t fill = is_active
            ? menu_mix_565(COL_DEEP_BLK, accent, 0.075f)
            : COL_DEEP_BLK;
        uint16_t stroke = is_active
            ? menu_mix_565(edge, COL_WHITE, 0.16f)
            : menu_mix_565(COL_WHITE, COL_BLACK, 0.88f);

        draw_menu_ring_segment(buf, deg, sweep, inner_r, outer_r, fill, stroke, is_active);
        draw_menu_arc(buf, outer_r - 2, deg, sweep - 8.0f, is_active ? 7 : 4, menu_mix_565(edge, COL_BLACK, is_active ? 0.14f : 0.72f));
        draw_menu_arc(buf, outer_r - 2, deg, sweep - 8.0f, is_active ? 3 : 2, is_active ? edge : menu_mix_565(edge, COL_BLACK, 0.48f));
        draw_menu_arc_range(buf, outer_r - 10, deg - sweep * 0.5f + 7.0f,
                            deg - sweep * 0.5f + 16.0f, 1,
                            is_active ? edge : menu_mix_565(edge, COL_BLACK, 0.55f));

        int icon_x = CENTER_X + (int)(154 * cosf(ang));
        int icon_y = CENTER_Y + (int)(154 * sinf(ang));
        draw_menu_icon(buf, icon_x, icon_y, i, is_active ? COL_WHITE : menu_mix_565(COL_WHITE, COL_BLACK, 0.42f));

        int label_x = CENTER_X + (int)(176 * cosf(ang));
        int label_y = CENTER_Y + (int)(176 * sinf(ang));
        const char *text = s_menu_items[i].cn_name;
        int tw = text_width_bgfg(text, 1);
        draw_text_bgfg(buf, text, label_x - tw / 2, label_y - 8,
                       COL_WHITE, is_active ? COL_WHITE : menu_mix_565(COL_WHITE, COL_BLACK, 0.46f),
                       1, is_active);
    }

    /* 中心机器人控制核 */
    fill_circle(buf, CENTER_X, CENTER_Y, 90, menu_mix_565(COL_BLACK, COL_WHITE, 0.035f));
    draw_circle(buf, CENTER_X, CENTER_Y, 90, menu_mix_565(COL_BRIGHT_GR, COL_BLACK, s_drag_active ? 0.50f : 0.70f));
    draw_circle(buf, CENTER_X, CENTER_Y, 73, menu_mix_565(COL_CYAN, COL_BLACK, 0.78f));
    draw_menu_avatar(buf, CENTER_X, CENTER_Y - 24, 92);

    if (s_drag_active && s_drag_idx >= 0) {
        const char *text = s_menu_items[s_drag_idx].cn_name;
        int tw = text_width_bgfg(text, 1);
        draw_text_bgfg(buf, text, CENTER_X - tw / 2, CENTER_Y + 48, COL_WHITE, COL_BRIGHT_GR, 1, 1);
    } else {
        int tw = text_width_sb("TRAE", 1);
        draw_text_sb(buf, "TRAE", CENTER_X - tw / 2, CENTER_Y + 46, COL_BRIGHT_GR, 1, 1);
        draw_text_sb(buf, "MENU", CENTER_X - text_width_sb("MENU", 1) / 2, CENTER_Y + 66,
                     menu_mix_565(COL_WHITE, COL_BLACK, 0.54f), 1, 0);
    }
}
