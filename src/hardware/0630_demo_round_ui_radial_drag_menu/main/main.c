 ...
 *   3. 菜单页: 按住中心 Logo 径向拖拽选择 6 个子菜单 (状态/蜘蛛/告警/能量/设置/关于)
...
static bool s_menu_drag_active = false;
static int s_menu_drag_x = CENTER_X;
static int s_menu_drag_y = CENTER_Y;
...
static void draw_menu(void)
{
    ...
    if (s_menu_drag_active) {
        draw_circle(buf, CENTER_X, CENTER_Y, MENU_CENTER_RADIUS, COL_BRIGHT_GR);
        if (s_menu_highlight >= 0 && s_menu_highlight < 6) {
            draw_line(buf, CENTER_X, CENTER_Y, s_menu_drag_x, s_menu_drag_y, COL_BRIGHT_GR);
            draw_line(buf, CENTER_X + 1, CENTER_Y, s_menu_drag_x + 1, s_menu_drag_y, COL_BRIGHT_GR);
        }
    }
    ...
    draw_text_center_sb(buf, "TRAE", CENTER_Y - 8, COL_TRAE_GRN, 1, 1);
...
static bool menu_is_center_handle(int tx, int ty)
{
    int dx = tx - CENTER_X;
    int dy = ty - CENTER_Y;
    return dx * dx + dy * dy <= MENU_CENTER_RADIUS * MENU_CENTER_RADIUS;
}

static int menu_select_by_direction(int tx, int ty)
{
    int drag_dx = tx - CENTER_X;
    int drag_dy = ty - CENTER_Y;
    int drag_dist2 = drag_dx * drag_dx + drag_dy * drag_dy;
    if (drag_dist2 < MENU_DRAG_SELECT_RADIUS * MENU_DRAG_SELECT_RADIUS) {
        return -1;
    }

    float drag_len = sqrtf((float)drag_dist2);
    int best_idx = -1;
    float best_score = -2.0f;

    for (int i = 0; i < 6; i++) {
        int item_dx = s_menu_items[i].x - CENTER_X;
        int item_dy = s_menu_items[i].y - CENTER_Y;
        float item_len = sqrtf((float)(item_dx * item_dx + item_dy * item_dy));
        if (item_len <= 0.0f) continue;

        float score = (float)(drag_dx * item_dx + drag_dy * item_dy) / (drag_len * item_len);
        if (score > best_score) {
            best_score = score;
            best_idx = i;
        }
...
static void enter_menu_item(int item_idx)
{
    if (item_idx < 0 || item_idx >= 6) return;

    s_current_page = s_menu_items[item_idx].target_page;
    s_menu_highlight = -1;
    s_menu_drag_active = false;
    ...
    ESP_LOGI(TAG, "径向菜单 → %s (页面 %d)",
             s_menu_items[item_idx].cn_name, s_menu_items[item_idx].target_page);
}
    ...
    while (1) {
        ...
        if (pressed) {
            ...
            if (!touching) {
                ...
                if (page == PAGE_MENU && menu_is_center_handle(tx, ty)) {
                    menu_dragging = true;
                    menu_drag_selected = -1;
                    xSemaphoreTake(s_state_mutex, portMAX_DELAY);
                    s_menu_drag_active = true;
                    s_menu_drag_x = tx;
                    s_menu_drag_y = ty;
                    s_menu_highlight = -1;
                    xSemaphoreGive(s_state_mutex);
                    ESP_LOGI(TAG, "中心菜单拖拽开始");
                } else {
                    menu_dragging = false;
                    menu_drag_selected = -1;
                    if (page == PAGE_MENU) {
            ...
            if (page == PAGE_MENU && !long_pressed) {
                if (menu_dragging) {
                    int selected = menu_select_by_direction(tx, ty);
                    menu_drag_selected = selected;
                    xSemaphoreTake(s_state_mutex, portMAX_DELAY);
                    s_menu_drag_active = true;
                    s_menu_drag_x = tx;
                    s_menu_drag_y = ty;
                    s_menu_highlight = selected;
                    xSemaphoreGive(s_state_mutex);
                }
#if MENU_ENABLE_OUTER_TAP
                else {
            ...
            int center_dx = last_x - CENTER_X;
            int center_dy = last_y - CENTER_Y;
            uint32_t duration = (xTaskGetTickCount() - touch_start_tick) * portTICK_PERIOD_MS;
            int dist2 = dx * dx + dy * dy;
            int center_dist2 = center_dx * center_dx + center_dy * center_dy;
            bool was_menu_dragging = menu_dragging;
            int selected = menu_drag_selected;
            ...
            /* Menu 页中心径向拖拽: 从中心出发, 松手进入当前方向子菜单 */
            if (was_menu_dragging) {
                if (center_dist2 >= MENU_DRAG_RELEASE_RADIUS * MENU_DRAG_RELEASE_RADIUS) {
                    if (selected < 0) {
                        selected = menu_select_by_direction(last_x, last_y);
                    }
                    if (selected >= 0) {
                        xSemaphoreTake(s_state_mutex, portMAX_DELAY);
                        enter_menu_item(selected);
                        xSemaphoreGive(s_state_mutex);
                    } else {
                        ESP_LOGI(TAG, "中心菜单拖拽取消: 未命中方向");
                    }
                } else {
                    ESP_LOGI(TAG, "中心菜单拖拽取消: 距离不足");
                }
                continue;
            }
