...
static void draw_menu(void)
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        int label_x = CENTER_X + (int)(183 * cosf(ang));
        int label_y = CENTER_Y + (int)(183 * sinf(ang));
        const char *text = s_menu_items[i].cn_name;
        int tw = text_width_bgfg(text, 1);
        draw_text_bgfg(buf, text, label_x - tw / 2, label_y - 8,
                       COL_GRAY, is_active ? COL_WHITE : menu_mix_565(COL_WHITE, edge, 0.36f),
                       1, is_active);
    }
                       ...
                       int used, const char *right_str, uint16_t color, int val_px)
{
    ...
    /* label 居左 (双通道: 中文16px灰白 + 无ASCII) */
    draw_text_bgfg(buf, label, Q_METER_LABEL_X, y - 8, COL_GRAY, COL_WHITE, 1, 0);
    ...
    /* right_str 最右 (双通道: 数字亮 + 中文灰白) */
    draw_text_bgfg(buf, right_str, Q_METER_RIGHT_X, y - 8, COL_GRAY, COL_WHITE, val_sc, 0);
}
...
static void draw_quota(void)
{
    ...
    if (elapsed >= QUOTA_ARC_ANIM_MS) {
        ...
        /* ease-out cubic: 1-(1-t)^3, 20帧平滑过渡 */
    ...
    /* ---- header: TRAE WORK (居中, y=Q_HEADER_Y, px→scale, bold, 绿色渐变) ---- */
    {
        ...
        for (int i = 0; i < n; i++) {
            ...
            uint16_t col = color_mix_565(COL_DARK_GR, COL_BRIGHT_GR, t);
    ...
    /* ---- 套餐小字: 速通 PRO+ 单月 (双通道: 中文16px灰白 + ASCII前景) ---- */
    draw_text_bgfg_center(buf, "速通 PRO+ 单月", Q_PLAN_Y, COL_GRAY, COL_WHITE, SC_FROM_PX(Q_PLAN_PX), 0);
    ...
    /* ---- 中心主信息: 可用 132 次 (双通道混合: 中文16px灰白 + 数字亮绿前景) ---- */
    {
        char num[8];
        ...
        draw_text_bgfg_center(buf, line, Q_MAIN_Y, COL_GRAY, COL_BRIGHT_GR, sc, 1);
    }

    /* ---- 日期时间 (双通道: 中文16px灰白 + ASCII灰白前景, 内联绘制, 实时更新) ---- */
    {
        const char *wd_names[] = {"周日","周一","周二","周三","周四","周五","周六"};
        char line[32];
        snprintf(line, sizeof(line), "%02d:%02d %02d/%02d %s",
        ...
        draw_text_bgfg_center(buf, line, y, COL_GRAY, COL_GRAY, 1, 0);
    }
}
