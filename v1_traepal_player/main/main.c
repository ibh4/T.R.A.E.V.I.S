/* traepal_player - TraePal 状态显示版 (动画 + WiFi 状态切换)
 * 功能: WiFi STA 连接家里 WiFi, 接收状态命令, 播放对应表情动画 + 底部状态文字
 * 方案: raw partition 存全屏 RGB565 帧, esp_partition_read 按偏移读取,
...
static uint8_t *s_frame_buf;          /* PSRAM 单帧缓冲 */
static esp_lcd_panel_handle_t s_panel = NULL;
static const esp_partition_t *s_frames_part = NULL;  /* raw 帧分区 */
static traepal_state_t s_current_state = STATE_IDLE;
static SemaphoreHandle_t s_state_mutex;  /* 保护 s_current_state */
...
/* 从 raw 分区读一帧全屏 RGB565 到 s_frame_buf
 * 布局: [idle_ready 8帧][bug_alert 8帧][fix_success 8帧], 每帧 FRAME_BYTES
 */
static bool load_frame(traepal_state_t state, int frame_idx)
{
    ...
    uint32_t offset = ((uint32_t)state * FRAMES_PER_STATE + frame_idx) * FRAME_BYTES;
    esp_err_t ret = esp_partition_read(s_frames_part, offset, s_frame_buf, FRAME_BYTES);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "esp_partition_read 失败: offset=%u %s",
