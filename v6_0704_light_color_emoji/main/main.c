...
#define COL_LIGHT_GRAY 0xC618 /* 浅灰 (时钟页日期) */
#define COL_RED       0xF800
#define COL_YELLOW    0xFFE0
#define COL_CYAN      0x07FF
#define COL_ORANGE    0xFD20  /* emoji 闪电外发光 */
#define COL_BROWN     0x9260  /* emoji 蜘蛛腿 */
#define COL_BLUE      0x1B9F  /* emoji 关于圆 */
#define COL_SKYBLUE   0x45BF  /* emoji 齿轮中心 */
...
/* 旋转绘制单个中文字 (16×16 降采样, 绕字符中心旋转 rot_rad 弧度)
 * 用于菜单标签沿圆弧贴边排版 */
static void draw_chinese_char_rotated(uint16_t *buf, const char *utf8,
                                       float cx, float cy, float rot_rad,
                                       uint16_t color)
{
    int idx = find_chinese_index(utf8);
    if (idx < 0 || !s_font_part) return;
    uint8_t mask[1024];
    if (esp_partition_read(s_font_part, (uint32_t)idx * 1024, mask, 1024) != ESP_OK) return;

    float cos_r = cosf(rot_rad);
    float sin_r = sinf(rot_rad);

    /* 16×16 降采样 (2×2 取最大值), 逐像素旋转绘制 */
    for (int row = 0; row < 16; row++) {
        for (int col = 0; col < 16; col++) {
            ...
            uint8_t a = a0 > a1 ? a0 : a1;
            a = a > a2 ? a : a2;
            a = a > a3 ? a : a3;
            if (a > 80) {
                float ox = col - 7.5f;
                float oy = row - 7.5f;
                int px = (int)(cx + ox * cos_r - oy * sin_r);
                int py = (int)(cy + ox * sin_r + oy * cos_r);
                if (px >= 0 && px < LCD_H_RES && py >= 0 && py < LCD_V_RES)
                    buf[py * LCD_H_RES + px] = color;
            }
        }
...
/* 沿圆弧贴边排版菜单标签 (逐字计算坐标并逐字旋转)
 * 顶部/底部(deg≈±90) 水平正读; 左右沿切线旋转, 上半字底朝圆心, 下半字顶朝圆心 */
static void draw_arc_label(uint16_t *buf, const char *text,
                            float ang, float label_r, uint16_t color, int bold)
{
    (void)bold;
    float deg = ang * 180.0f / (float)M_PI;
    float abs_deg = fabsf(deg);
    int is_horizontal = (abs_deg > 60.0f && abs_deg < 120.0f);

    if (is_horizontal) {
        int ccx = CENTER_X + (int)(label_r * cosf(ang));
        int ccy = CENTER_Y + (int)(label_r * sinf(ang));
        const char *p = text;
        int ci = 0;
        while (*p) {
            uint8_t c = (uint8_t)*p;
            if ((c & 0xF0) == 0xE0) {
                int char_count = 2;
                int total_w = char_count * 18 - 2;
                int ox = -total_w / 2 + ci * 18 + 8;
                draw_chinese_char_rotated(buf, p, (float)(ccx + ox - 8), (float)ccy, 0.0f, color);
                ci++;
                p += 3;
            } else {
                p++;
        ...
        float rot_deg = (deg < 0.0f) ? (deg + 90.0f) : (deg - 90.0f);
        float rot_rad = rot_deg * (float)M_PI / 180.0f;
        float char_sep = 10.0f * (float)M_PI / 180.0f;
        const char *p = text;
        int ci = 0;
        int char_count = 2;
        while (*p) {
            uint8_t c = (uint8_t)*p;
            if ((c & 0xF0) == 0xE0) {
                float char_ang = ang + (ci - (char_count - 1) / 2.0f) * char_sep;
                float ccx = CENTER_X + label_r * cosf(char_ang);
                float ccy = CENTER_Y + label_r * sinf(char_ang);
                draw_chinese_char_rotated(buf, p, ccx, ccy, rot_rad, color);
                ci++;
                p += 3;
            } else {
                p++;
...
{
    /* 彩色 emoji 风格: 始终多色绘制, 不再依赖传入 color (color 仅用于 active 时加亮外发光) */
    int s = 16;
    (void)color;  /* 保留参数兼容, 不再用于主色 */
    switch (idx) {
        case 0: { /* 状态 - 仪表盘: 白弧 + 绿刻度 + 红指针 + 黑中心 */
            for (int a = 180; a <= 360; a++) {
                float rad = a * M_PI / 180.0f;
                int x = cx + (int)(s * cosf(rad));
                int y = cy + (int)(s * sinf(rad));
                if (x >= 0 && x < LCD_H_RES && y >= 0 && y < LCD_V_RES)
                    buf[y * LCD_H_RES + x] = COL_WHITE;
            }
            /* 绿色刻度 */
            for (int a = 180; a <= 360; a += 30) {
                float rad = a * M_PI / 180.0f;
                int x1 = cx + (int)((s - 4) * cosf(rad));
                int y1 = cy + (int)((s - 4) * sinf(rad));
                int x2 = cx + (int)(s * cosf(rad));
                int y2 = cy + (int)(s * sinf(rad));
                draw_line(buf, x1, y1, x2, y2, COL_TRAE_GRN);
            }
            /* 红色指针 */
            draw_line(buf, cx, cy, cx + s * 2 / 3, cy - s / 2, COL_RED);
            fill_circle(buf, cx, cy, 2, COL_BLACK);
            break;
        }
        case 1: { /* 蜘蛛 - 棕腿 + 黑身 + 红眼 */
            for (int a = 0; a < 360; a += 45) {
                ...
                int x2 = cx + (int)(s * cosf(rad));
                int y2 = cy + (int)(s * sinf(rad));
                draw_line(buf, x1, y1, x2, y2, COL_BROWN);
            }
            fill_circle(buf, cx, cy, 6, COL_BLACK);
            /* 红色眼睛 */
            fill_circle(buf, cx - 2, cy - 1, 1, COL_RED);
            fill_circle(buf, cx + 2, cy - 1, 1, COL_RED);
            break;
        }
        case 2: { /* 告警 - 黄三角 + 黑! */
            for (int dy = -s; dy <= s; dy++) {
                ...
                if (abs(dy) > 2 && abs(dy) < s - 1) {
                    draw_hline(buf, cx - hw / 2, cx + hw / 2, cy + dy, COL_YELLOW);
                }
            }
        ...
        case 3: { /* 能量 - 橙外发光 + 黄主体 */
            /* 橙色外发光 (偏移 1px) */
            draw_line(buf, cx - 1, cy - s - 1, cx + 5, cy - 1, COL_ORANGE);
            draw_line(buf, cx + 5, cy - 1, cx - 1, cy + 1, COL_ORANGE);
            draw_line(buf, cx - 1, cy + 1, cx + 5, cy + 3, COL_ORANGE);
            draw_line(buf, cx + 5, cy + 3, cx - 1, cy + s + 1, COL_ORANGE);
            /* 黄色主体 */
            draw_line(buf, cx - 2, cy - s, cx + 4, cy - 2, COL_YELLOW);
            draw_line(buf, cx + 4, cy - 2, cx - 2, cy, COL_YELLOW);
            draw_line(buf, cx - 2, cy, cx + 4, cy + 2, COL_YELLOW);
            draw_line(buf, cx + 4, cy + 2, cx - 2, cy + s, COL_YELLOW);
            break;
        }
        case 4: { /* 设置 - 灰齿 + 天蓝中心 + 白孔 */
            fill_circle(buf, cx, cy, 7, COL_DIM_GRAY);
            fill_circle(buf, cx, cy, 5, COL_GRAY);
            for (int a = 0; a < 360; a += 45) {
                float rad = a * M_PI / 180.0f;
                int tx = cx + (int)(10 * cosf(rad));
                int ty = cy + (int)(10 * sinf(rad));
                fill_rect(buf, tx - 1, ty - 1, 3, 3, COL_GRAY);
            }
            fill_circle(buf, cx, cy, 3, COL_SKYBLUE);
            fill_circle(buf, cx, cy, 1, COL_WHITE);
            break;
        }
        case 5: { /* 关于 - 蓝底 + 白 i */
            fill_circle(buf, cx, cy, s, COL_BLUE);
            draw_circle(buf, cx, cy, s, COL_WHITE);
            fill_rect(buf, cx - 1, cy - 7, 2, 3, COL_WHITE);
            fill_rect(buf, cx - 1, cy - 2, 2, 8, COL_WHITE);
            break;
        }
        default:
            break;
...
static void __attribute__((unused)) draw_menu_reticle(uint16_t *buf, int cx, int cy, int r, uint16_t color)
{
...
static void draw_gradient_aura(uint16_t *buf, int cx, int cy, int max_r, float frame_phase);
...
static void draw_menu(void)
{
    ...
    /* 底层渐变彩色氛围: 中心→外侧多彩渐变, 模糊感, 颜色偏深
     * 色相随帧缓慢流动 (每帧 +6°, 12 帧一圈 72°) */
    float aura_phase = (float)s_ready_frame * 6.0f;
    draw_gradient_aura(buf, CENTER_X, CENTER_Y, SCREEN_RADIUS - 4, aura_phase);

    /* HTML 原型 menu 形态: 环形扇面 + 顶/底两段粗外弧 (默认黑色, 拖拽时变色) */
    draw_menu_arc_range(buf, 212, 208, 332, 2, COL_DEEP_BLK);
    draw_menu_arc_range(buf, 212, -56, 58, 2, COL_DEEP_BLK);
    draw_circle(buf, CENTER_X, CENTER_Y, 116, COL_DEEP_BLK);
    draw_circle(buf, CENTER_X, CENTER_Y, 207, COL_DEEP_BLK);

    /* 6 个环形扇区按钮 (默认黑色, 拖拽到该方向时变色) */
    for (int i = 0; i < 6; i++) {
        ...
        /* 非 active: 全黑; active: 彩色 accent */
        uint16_t edge = is_active ? accent : COL_BLACK;
        ...
        draw_menu_arc(buf, outer_r - 2, deg, sweep - 8.0f, is_active ? 4 : 2, is_active ? menu_mix_565(edge, COL_BLACK, 0.04f) : COL_BLACK);
        draw_menu_arc(buf, outer_r - 2, deg, sweep - 8.0f, is_active ? 2 : 1, is_active ? edge : COL_BLACK);
        draw_menu_arc_range(buf, outer_r - 10, deg - sweep * 0.5f + 7.0f,
                            deg - sweep * 0.5f + 16.0f, 1, is_active ? edge : COL_BLACK);

        int icon_x = CENTER_X + (int)(120 * cosf(ang));
        int icon_y = CENTER_Y + (int)(120 * sinf(ang));
        /* 图标始终彩色 emoji 风格 (不再随 active 切换灰/白) */
        draw_menu_icon(buf, icon_x, icon_y, i, COL_WHITE);

        /* 中文标签沿圆弧贴边排版 (逐字旋转, r=160, 白色) */
        const char *text = s_menu_items[i].cn_name;
        draw_arc_label(buf, text, ang, 160.0f, COL_WHITE, is_active);
    }

    /* 中心控制核: 缩小版 ready 动图 (与额度页动图同尺寸 Q_AVATAR_SIZE, 圆裁) */
    {
        int anim_size = Q_AVATAR_SIZE;  /* 150, 与额度页一致 */
        int orbR = Q_ORB_R;             /* 75, 3D 球体半径 */
        /* 3D 球体背景 (与额度页同款) */
        draw_avatar_orb(buf, CENTER_X, CENTER_Y, orbR, COL_TRAE_GRN);
        /* idle_ready 动图 (状态 0), 圆裁到 anim_size */
        if (s_states_part && load_state_frame(0, s_ready_frame)) {
            ...
            for (int y = 0; y < anim_size; y++) {
                ...
                for (int x = 0; x < anim_size; x++) {
                    ...
                    int px = CENTER_X - anim_size / 2 + x;
                    int py = CENTER_Y - anim_size / 2 + y;
                    if (px >= 0 && px < LCD_H_RES && py >= 0 && py < LCD_V_RES)
        ...
        s_ready_frame = (s_ready_frame + 1) % FRAMES_PER_STATE;
        /* 已去除 draw_rainbow_ring, 改用底层 draw_gradient_aura 多彩氛围 */
    }
}
...
/* 菜单底层渐变彩色氛围: 从中心到外侧的径向 + 角度多彩渐变
 * - 多色切换: 色相 = 角度 + 半径贡献 + 时间相位
 * - 颜色偏浅: value 0.62→0.40 (中心亮, 边缘适中)
 * - 模糊感: 2×2 块采样 + alpha 与背景混合 + 低饱和度
 * - 在 draw_background 之后调用, 作为底层氛围 */
static void draw_gradient_aura(uint16_t *buf, int cx, int cy, int max_r, float frame_phase)
{
    for (int y = 0; y < LCD_V_RES; y += 2) {
        for (int x = 0; x < LCD_H_RES; x += 2) {
            ...
            float t = dist / (float)max_r;          /* 0(中心) ~ 1(边缘) */
            /* 角度 + 半径 + 时间相位 → 色相 (多彩流动) */
            float ang = atan2f((float)dy, (float)dx) * 180.0f / (float)M_PI;
            if (ang < 0) ang += 360.0f;
            float h = fmodf(ang + t * 120.0f + frame_phase + 360.0f, 360.0f);
            /* 颜色偏浅: 中心 v=0.62 → 边缘 v=0.40 */
            float v = 0.62f - t * 0.22f;
            /* 饱和度适中, 避免过艳 */
            float s = 0.55f;
            uint16_t c = hsv_to_rgb565(h, s, v);
            /* alpha 从中心 0.70 → 边缘 0.35, 与背景混合形成朦胧模糊感 */
            float alpha = 0.70f - t * 0.35f;
            /* 2×2 块共用一色 (降采样 → 天然柔和模糊) */
            for (int oy = 0; oy < 2; oy++) {
                for (int ox = 0; ox < 2; ox++) {
                    int px = x + ox, py = y + oy;
                    if (px >= 0 && px < LCD_H_RES && py >= 0 && py < LCD_V_RES) {
                        uint16_t bg = buf[py * LCD_H_RES + px];
                        buf[py * LCD_H_RES + px] = menu_mix_565(bg, c, alpha);
                    }
                }
            }
        }
...
static void draw_watch(void)
{
    ...
    /* ---- 下方日期: YYYY/MM/DD 周X (scale=1, 灰白), y=340 (避开 TRAE 下移) ---- */
    {
        ...
        draw_text_bgfg_center(buf, line, 340, COL_LIGHT_GRAY, COL_LIGHT_GRAY, 1, 0);
    }
}
